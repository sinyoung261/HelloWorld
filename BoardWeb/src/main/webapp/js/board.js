...
function createPageList() {
	...
	fetch('getConut.do?bno=' + bno)
		.then(reault => reault.json())
		.then(result => {
			let totalCnt = result.replyCount;//댓글전체건수. 75건.(15페이지).
			// 이전, 이후 페이지여부, 첫번째~ 마지막 페이지.
			let prev, next; //이전, 이후
			let startPage, endPage, realEnd;
			endPage = Math.ceil(page / 10) * 10; //현재 페이지를 기준으로 마지막페이지 계산.
...
document.querySelector('#addBtn').addEventListener('click', function(e) {
	...
	fetch('addReply.do?bno=' + bno + '&replyer=' + logId + '&reply=' + reply)
		.then(result => result.json())
		.then(result => {
			console.log(result);//결과학인.retCode:OK
